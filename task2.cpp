        ...
        //Get the number of faces for each image
             ...
             // Store the face parameters
              ...
              // Calculate truePositives
							for( int i = 0; i < darts.size(); i++ ){
								 for(int j = 0; j < nDarts; j++) {
									 if(abs(darts[i].x - truth[j][0]) < 10*truth[j][0] / 100 &&
											abs(darts[i].y - truth[j][1])  < 10*truth[j][1] / 100  &&
											abs(darts[i].height -truth[j][3]) < 10*truth[j][3] / 100 ) {
											truePositives++;
										}
									}
							 }
              ...
              //cout << "True positive rate is " << tpr << "\n";
							...
							// Calculate F1 score
							falsePositives = darts.size() - nDarts;
